<template>
  <BasicLayout>
    <div>
      <h2>Editar ASN <span style="color:blue"> {{$route.params.asn}}</span></h2>
      <form class="ui form" @submit.prevent="editarAsnC">
...
import { editarAsn, getInfoAsn} from "../api/api";
...
export default {
  ...
  setup() {
    ...
    const getAsnInfoC = async (org) => {
      const response = await getInfoAsn(org);
      console.log("ACÁ" + JSON.stringify(response));
      formData.value = response;
    };

    const editarAsnC = async () => {
      formError.value = {};
      ...
      try {
        ...
        try {
          ...
          const response = await editarAsn(formData.value, params.asn);
